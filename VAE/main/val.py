...
output_path = os.path.join('exp', f"z{z_dim}")
...
for i in tqdm(range(num_epoch)):
    ...
    if (i+1) % i_val == 0:
        vis_lat(i+1, f"./{output_path}/val", model, z_dim, writer, device)
        test(i+1, f"./{output_path}/val", test_loader, model, device, in_dim, z_dim, beta, writer)
        vis_img(i+1, f"./{output_path}/val", vis_data.to(device), model, writer)

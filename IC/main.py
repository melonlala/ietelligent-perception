...
from utils.util import get_logger, get_sampling_probability
...
class Runner(object): 
    ...
    def train(self, config_file, **kwargs):
        ...
        state = torch.load("/GPFS/rhome/yuchenhou/workspace/Data/image_captioning/experiments/resnet101_attention_linear/resnet101_attention_b128_emd300_best_val.pt", map_location="cpu")
        ...
        method = args['schedule_sampling']
        k = args['sample_k']
        for epoch in range(num_epochs):
            p = get_sampling_probability(num_epochs, epoch, method, k)
        ...
        with torch.no_grad():
            ...
            with open(f"{model_path}_coco_scores.txt", "w") as writer:
                for scorer in scorers:
                    ...
                    examples[method] = {
                        "high": {"img_id": imgids[max_index], "prediction": predictions[max_index], "score": scores[max_index]},
                        "low": {"img_id": imgids[min_index], "prediction": predictions[min_index], "score": scores[min_index]}
                    }
                    print(f'example for {method}', examples[method])
                output["SPIDEr"] /= 2
                print(f"SPIDEr: {output['SPIDEr']:.3f}", file=writer)

            json.dump(output_pred, open(f"{model_path}_predictions.json", "w"), indent=4)
            with open(f"{model_path}_high_low_examples.json", "w") as f:
